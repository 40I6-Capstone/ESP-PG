...
def main():
    try:
        ...
        elif state == 'IDLE':  # wait for function call or a request from the network
        ...
        elif state == 'NODESTATE_SEND':  # send a nodestate packet
            node_list[node_index].conn.sendall(b'1')  # check 0th node by default, unless otherwise specified
            state = 'NODESTATE_RECEIVE'

        elif state == 'NODESTATE_RECEIVE':  # wait for response from node, once received, parse the data, and update internal current state
            buffer = b''
            while len(buffer) < 42:  # wait for the buffer to fill with packet data
                data = Node.conn.recvall(42)  # packet size of 42 bytes for state
                buffer += data
        ...
        elif state == 'SEND_PATH_PACKET':  # parse received data and update internal current state
            Node.conn.sendall(b'2')
            state = 'SENDPATH_RECEIVE'

        elif state == 'SENDPATH_AWAIT_REPLY':  # wait for ready signal from node
            buffer = b''
            not_ready = True
            while not_ready:
                while len(buffer) < 42:  # wait for the buffer to fill with packet data
        ...
        elif state == 'SENDPATH_SEND_STREAM':  # SEND STREAM OF BYTES FOR PATH
            # how do I take in path point data for this?
            path_packet = Packet()
            send_path(path_packet, Node)
            state = 'SENDPATH_AWAIT_CHECK'

        elif state == 'SENDPATH_AWAIT_CHECK':  # wait for packet received from node
            buffer = b''
